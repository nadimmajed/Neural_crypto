...
class model:
    ...
    def without_bias(self):
        ...
        np.random.seed(1337)
        model = Sequential()
        model.add(Dense(self.hidden_unit, input_dim=2, activation=self.activation1,kernel_initializer=initializers.RandomNormal(mean=self.mean, stddev=self.stddev, seed=None)))
