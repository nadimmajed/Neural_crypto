...
class modelB:
    ...
    def with_bias(self,i):
        ...
        model = Sequential()
        model.add(Dense(self.hidden_unit, input_dim=4, activation=self.activation1,
                        kernel_initializer=initializers.RandomNormal(mean=self.mean, 
                                                                     stddev=self.stddev, seed=None),bias_initializer='one'))
